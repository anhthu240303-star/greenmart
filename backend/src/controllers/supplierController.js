...
const getSupplierById = async (req, res) => {
  try {
    ...
    // Đếm tổng số giao dịch liên quan
  ...
  try {
    ...
    if (transactionCount > 0) {
      return ApiResponse.badRequest(
        res,
        `Không thể vô hiệu hóa nhà cung cấp vì có ${transactionCount} giao dịch liên quan`
      );
    }
...
const getSupplierStatistics = async (req, res) => {
  try {
    ...
    const products = [
      ...new Set(transactions.map((t) => t.product?._id?.toString())),
    ];
    ...
    return ApiResponse.success(
      res,
      statistics,
      'Lấy thống kê nhà cung cấp thành công'
    );
  } catch (error) {
